# Draft: Show and tell — Archie, a live guided tour + AI tutor for new Omarchy users

*(post as Luke, basecamp/omarchy Discussions; embed the demo video at top)*

---

**Archie teaches new users Omarchy the way Omarchy teaches everything else:
hands-on, keyboard-first, AI when it helps.**

[DEMO VIDEO/GIF HERE]

New users get a corner chat widget with an 11-step **live tour**: "Press
SUPER+RETURN"… and Archie *sees the window open* (Hyprland event socket), ticks
...
Install: `Install → AUR → archie-omarchy` (or `git clone … && ./install.sh`).
