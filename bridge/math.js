...
const functions = new Set([
  ...
  "median", "min", "mod", "permutations", "pow", "prod", "round", "sign", "sin",
  "sinh", "sqrt", "std", "sum", "tan", "tanh", "variance",
]);
const constants = new Set(["e", "pi", "tau", "phi"]);
// A number, unsigned. Callers add a sign where one is allowed.
const NUMBER = "(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:e[-+]?\\d+)?";
const SIGNED_NUMBERS = new RegExp(`[-+]?${NUMBER}`, "gi");
...
function resourceSafe(tree) {
  ...
  tree.traverse((node) => {
    ...
    if (args.length > 200) { safe = false; return; }
...
function safeExpression(tree, conversion) {
  let safe = true;
...
function calculate(input) {
  ...
  const expression = command || conversion || arithmeticExpression(normalizedText);
  if (!expression) return null;
  try {
    const tree = parse(expression);
    if (!safeExpression(tree, Boolean(conversion)) || !resourceSafe(tree)) return null;
